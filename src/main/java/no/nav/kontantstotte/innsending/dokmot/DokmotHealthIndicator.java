...
public class DokmotHealthIndicator implements HealthIndicator {
    ...
    public Health health() {
        if (!isDev()) {
            try {
                ...
                LOG.warn("Could not verify health of DOKMOT {}", pinger.toString(), e);
    ...
    private Health downWithDetails(Exception e) {
        return Health.down().withDetail("config", pinger.toString()).withException(e).build();
    }
    ...
    private Health upWithDetails() {
        return Health.up().withDetail("config", pinger.toString()).build();
    }
